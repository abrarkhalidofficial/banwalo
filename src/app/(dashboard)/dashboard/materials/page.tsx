...
export default function MaterialsPage() {
  ...
  const getAvailableQuantity = (materialId: Id<"materials">): number => {
    return stockEntries
      ...
      .reduce((total, entry) => total + entry.remainingQuantity, 0);
  };
  ...
  const filteredStockEntries = stockEntries.filter(entry =>
    entry.materialName.toLowerCase().includes(stockSearchTerm.toLowerCase()) ||
    entry.supplierName.toLowerCase().includes(stockSearchTerm.toLowerCase())
  );
                  ...
                  {filteredMaterials.length === 0 ? (
                    ...
                    filteredMaterials.map((material) => (
                      ...
                      >
                        ...
                        <TableCell>
                          {getAvailableQuantity(material._id)} units
                        </TableCell>
                        <TableCell>
                          {material.lowStockThreshold ? `${material.lowStockThreshold} units` : "-"}
                        </TableCell>
                      </TableRow>
                    ))
                  )}
                  ...
                  {filteredStockEntries.length === 0 ? (
                    ...
                    filteredStockEntries.map((entry) => (
                      ...
                      >
                        ...
                        <TableCell>
                          <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${getStatusStyles(getStockStatus(entry))}`}>
                            {getStockStatus(entry)}
                          </span>
                        </TableCell>
                      </TableRow>
                    ))
