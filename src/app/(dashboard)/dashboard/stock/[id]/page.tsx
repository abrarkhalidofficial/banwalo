...
import { ArrowLeft, AlertTriangle, ShoppingCart, Factory, Package, Scissors } from "lucide-react";
...
// Define types for consumption history
...
export default function StockEntryDetailPage() {
  ...
  const isLowStock = stockEntry.material?.lowStockThreshold && 
    stockEntry.remainingQuantity <= stockEntry.material.lowStockThreshold;
  ...
  const handleNavigateToProduction = (productionId: Id<"productions">) => {
            ...
            <dl className="divide-y divide-gray-200">
              ...
              <div className="py-3 grid grid-cols-3">
                ...
                <dd className="text-sm text-gray-900 col-span-2">
                  <button 
                    onClick={() => handleNavigateToSupplier(stockEntry.supplierId)}
              ...
              <div className="py-3 grid grid-cols-3">
                ...
                <dd className="text-sm text-gray-900 col-span-2">₹{stockEntry.pricePerUnit.toFixed(2)}</dd>
              </div>
              <div className="py-3 grid grid-cols-3">
                <dt className="text-sm font-medium text-gray-500">Total Value</dt>
                <dd className="text-sm text-gray-900 col-span-2">
                  ₹{(stockEntry.quantity * stockEntry.pricePerUnit).toFixed(2)}
                </dd>
              </div>
              <div className="py-3 grid grid-cols-3">
                <dt className="text-sm font-medium text-gray-500">Remaining Value</dt>
                <dd className="text-sm text-gray-900 col-span-2">
                  ₹{(stockEntry.remainingQuantity * stockEntry.pricePerUnit).toFixed(2)}
                </dd>
              </div>
              {stockEntry.notes && (
                <div className="py-3 grid grid-cols-3">
                  <dt className="text-sm font-medium text-gray-500">Notes</dt>
                  <dd className="text-sm text-gray-900 col-span-2">{stockEntry.notes}</dd>
                </div>
              )}
            </dl>
          </CardContent>
            ...
            <div className="mt-6 space-y-3">
              ...
              {!isDepleted && (
                <Button 
                  className="w-full" 
                  variant="outline"
                  onClick={() => router.push("/dashboard/stock/consume")}
                >
            ...
            <TableBody>
              {consumptionHistory.length === 0 ? (
                ...
                consumptionHistory.map((usage) => (
                  <TableRow 
                    key={`${usage.productionId}-${usage.dateUsed}`}
                    className="cursor-pointer hover:bg-muted/50"
                    onClick={() => handleNavigateToProduction(usage.productionId)}
                  >
