...
export const getTotalExpensesForMonth = query({
  args: {
    month: v.number(), // 0-indexed month (0 for January, 11 for December)
    year: v.number(),
  },
  handler: async (ctx, { month, year }) => {
    ...
    const expensesInMonth = await ctx.db
      .query("expenses")
      .filter((q) =>
        q.and(q.gte(q.field("date"), startOfMonth), q.lte(q.field("date"), endOfMonth))
      )
      .collect();
...
export const getMonthlyExpenseSummaries = query({
  handler: async (ctx) => {
    ...
    const monthlySummaries: { [key: string]: { year: number; month: number; totalAmount: number } } = {};
