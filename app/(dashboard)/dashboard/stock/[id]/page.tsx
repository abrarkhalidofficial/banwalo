          ...
          <CardContent>
            <dl className="divide-y divide-gray-200">
              ...
              <div className="py-3 grid grid-cols-3">
                ...
                <dd className="text-sm text-gray-900 col-span-2">PKR {(stockEntry.pricePerUnit ?? 0).toFixed(2)}</dd>
              </div>
              <div className="py-3 grid grid-cols-3">
                <dt className="text-sm font-medium text-gray-500">Total Value</dt>
                <dd className="text-sm text-gray-900 col-span-2">PKR {(stockEntry.quantity * (stockEntry.pricePerUnit ?? 0)).toFixed(2)}</dd>
              </div>
              <div className="py-3 grid grid-cols-3">
                <dt className="text-sm font-medium text-gray-500">Remaining Value</dt>
                <dd className="text-sm text-gray-900 col-span-2">PKR {(stockEntry.remainingQuantity * (stockEntry.pricePerUnit ?? 0)).toFixed(2)}</dd>
              </div>
            </dl>
          </CardContent>
        </Card>
              ...
              {consumptionHistory.length === 0 ? (
                ...
                consumptionHistory.map((usage) => (
                  <TableRow key={`${usage.productionId}-${usage.dateUsed}`} className="cursor-pointer hover:bg-muted/50" onClick={() => handleNavigateToProduction(usage.productionId)}>
                    ...
                    <TableCell>PKR {usage.cost.toFixed(2)}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
