...
type StockStatus = 'Depleted' | 'Low' | 'Available';
...
export default function MaterialsPage() {
  ...
  const materials = (useQuery(api.materials.listWithLatestPrices) || []) as Material[];
  const stockEntries = (useQuery(api.stockEntries.list) || []) as StockEntry[];
                  ...
                  {filteredMaterials.length === 0 ? (
                    ...
                    filteredMaterials.map((material) => (
                      <TableRow key={material._id} className="cursor-pointer hover:bg-muted/50" onClick={() => handleMaterialClick(material._id)}>
                        ...
                        <TableCell>{getAvailableQuantity(material._id)} {material.unit || 'units'}</TableCell>
                        <TableCell>{material.latestPrice ? `PKR ${material.latestPrice}` : '-'}</TableCell>
                        <TableCell>{material.lowStockThreshold ? `${material.lowStockThreshold} ${material.unit || 'units'}` : '-'}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
                  ...
                  {filteredStockEntries.length === 0 ? (
                    ...
                    filteredStockEntries.map((entry) => (
                      <TableRow key={entry._id} className="cursor-pointer hover:bg-muted/50" onClick={() => handleStockEntryClick(entry._id)}>
                        ...
                        <TableCell>{entry.remainingQuantity} {entry.material?.unit || 'units'}</TableCell>
                        <TableCell>{entry.quantity} {entry.material?.unit || 'units'}</TableCell>
                        <TableCell>PKR {entry.pricePerUnit}</TableCell>
