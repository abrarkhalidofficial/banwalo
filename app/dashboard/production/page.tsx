...
export default function ProductionPage() {
  ...
  const rawProductions = useQuery(api.productions.list);

  type Production = NonNullable<typeof rawProductions>[0];

  const productions = useMemo(() => rawProductions || [], [rawProductions]);
  ...
  const handleCellChange = (rowIdx: number, field: keyof Production, value: string | number | undefined) => {
    setEditRows((prev) => {
      const updated = [...prev];
      updated[rowIdx] = {
        ...updated[rowIdx],
  ...
  const handleBulkSave = async () => {
    try {
      const updates = Object.entries(changedRows).map(([_id, changes]) => ({
        _id: _id as Id<'productionOrders'>,
        ...changes,
      }));
      await bulkUpdateProductions({ userId, updates });
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-8">
        ...
        <Button onClick={() => router.push('/dashboard/production/new')}>
          <PlusCircle className="mr-2 h-4 w-4" /> New Production{' '}
        </Button>
      </div>
        ...
        <CardHeader>
          ...
          <div className="flex gap-2 mt-2">
            {!isEditing ? (
              ...
              <>
                <Button variant="default" onClick={handleBulkSave}>
                  Save All
                </Button>
              ...
              {filteredProductions.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={24} className="text-center py-6 text-muted-foreground">
                    {productions.length === 0 ? 'No productions found. Add your first production to get started.' : 'No productions match your search criteria.'}
                  </TableCell>
                </TableRow>
              ) : (
                (isEditing ? editRows : filteredProductions).map((production, rowIdx) => (
                  <TableRow key={production._id} className={isEditing ? '' : 'cursor-pointer hover:bg-muted/50'} onClick={() => !isEditing && router.push(`/dashboard/production/${production._id}`)}>
                    <TableCell className="font-medium">
                      {isEditing ? <Input value={production.articleName} onChange={(e) => handleCellChange(rowIdx, 'articleName', e.target.value)} /> : production.articleName}
                    </TableCell>
                    <TableCell>{isEditing ? <Input value={production.clientName} onChange={(e) => handleCellChange(rowIdx, 'clientName', e.target.value)} /> : production.clientName}</TableCell>
                    <TableCell>{isEditing ? <Input value={production.type} onChange={(e) => handleCellChange(rowIdx, 'type', e.target.value)} /> : production.type}</TableCell>
                    <TableCell>
                      {isEditing ? (
                    ...
                    <TableCell>
                      {isEditing ? (
                        <Input
                          type="date"
                          value={production.cuttingDate ? new Date(production.cuttingDate).toISOString().slice(0, 10) : ''}
                          onChange={(e) => handleCellChange(rowIdx, 'cuttingDate', e.target.value ? new Date(e.target.value).getTime() : undefined)}
                        />
                      ) : (
                    ...
                    <TableCell>
                      {isEditing ? (
                        ...
                        <span
                          className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${production.status === 'Completed' || production.status === 'Delivered' ? 'bg-green-100 text-green-800' : production.status === 'In Progress' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800'}`}
                        >
                          {production.status}
                        </span>
                      )}
                    ...
                    <TableCell>
                      {isEditing ? (
                        ...
                        `PKR ${(production.clientPrice ?? 0).toFixed(2)}`
                      )}
                    </TableCell>
                    <TableCell>
                      {isEditing ? (
                    ...
                    <TableCell>
                      {isEditing ? (
                        ...
                        `PKR ${(production.printingCost ?? 0).toFixed(2)}`
                      )}
                    </TableCell>
                    <TableCell>
                      {isEditing ? (
                        ...
                        `PKR ${(production.pocketZipCost ?? 0).toFixed(2)}`
                      )}
                    </TableCell>
                    <TableCell>
                      {isEditing ? (
                        ...
                        `PKR ${(production.doryCost ?? 0).toFixed(2)}`
                      )}
                    </TableCell>
                    <TableCell>
                      {isEditing ? (
                        ...
                        `PKR ${(production.fullZipCost ?? 0).toFixed(2)}`
                      )}
                    </TableCell>
                    <TableCell>
                      {isEditing ? (
                        ...
                        `PKR ${(production.elasticCost ?? 0).toFixed(2)}`
                      )}
                    </TableCell>
                    <TableCell>
                      {isEditing ? (
                        ...
                        `PKR ${(production.packingZipperCost ?? 0).toFixed(2)}`
                      )}
                    </TableCell>
                    <TableCell>
                      {isEditing ? (
                        ...
                        `PKR ${(production.packingShopperCost ?? 0).toFixed(2)}`
                      )}
                    </TableCell>
                    <TableCell>
                      {isEditing ? (
                        ...
                        `PKR ${(production.threadCost ?? 0).toFixed(2)}`
                      )}
                    </TableCell>
                  </TableRow>
                ))
